...
#summary_result <- summary(first_model)

coefficients <- coef(first_model)
# Extract variable names (excluding intercept)
variables <- names(coefficients)[-1]  # Exclude the intercept

# Extract coefficients (excluding intercept)
coefficients <- coefficients[-1]  # Exclude the intercept

# Create data frame
coefs_df <- data.frame(Party = variables, Coefficients = coefficients)
...
It is worth stating that the reference ctaegory variable here is Alliance Party of Northern Ireland (APNI), which has holds an anti-Brexit stance.
...
This paper lays the groundwork for a more detailed study into which parties played the strongest role in Brexit. With the dataset of speeches already avaiable, devising an Intrumental Variable Framework to determine party influence seems like a suitable way forward.

As previously discussed, since speech occurence may not be a robust proxy for disagreement, employing an Instrumental Variable strategy will get rid of the reverse causality concerns.

### Instrumental Variable

When studying the relationship between two variables, there is a concern of reverse causality. For example, if we were to study the impact on institutions on the GDP of a nation, our estimates would be biased since GDP could also affect and improve an institituion. To tackle this, an Instrumental Variable approach can be used.
